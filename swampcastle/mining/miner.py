...
_GENERATED_SIGNATURES = re.compile(
    r"(?i)"
    r"(<auto-generated>|"  # typical auto-gen header
    r"code generated by|"  # go protobuf, grpc, etc
    r"sourceMappingURL=|"  # JS source maps
    r"webpackJsonp|"  # webpack bundles
    r"For license information please see|"  # license banner from bundlers
    r"Do not edit|"  # autogenerated
    r"@generated)"
)
...
def is_probably_minified(filepath: Path, content: str) -> bool:
    ...
    Rules (in order):
    1. Unconditional: if any line longer than LONGEST_LINE_REJECT -> reject
    2. Filename heuristics (.min.js, .bundle.js, .map) -> reject
    3. Generated signatures in first/last 1k chars -> reject
    4. For non-.json files: whitespace ratio, avg line length, symbol/alnum ratio
    5. JSON files: exempt from whitespace/ratio checks (JSON often compact but useful)
    """
    if content is None:
        return False

    # 1) unconditional long line check
    if _longest_line_length(content) > LONGEST_LINE_REJECT:
        return True
    ...
    # 3) generated signatures in head/tail
    head_tail = (content[:1000] + content[-1000:]) if len(content) > 2000 else content
    if _GENERATED_SIGNATURES.search(head_tail):
        return True

    # 4) heuristics for non-json files
    if suffix == ".json":
        return False
    ...
    if _avg_line_length(content) > AVG_LINE_LENGTH_THRESHOLD:
        return True

    if _symbol_to_alnum_ratio(content) > 1.5:
        return True
...
def process_file(
    ...
    content = content.strip()
    # Early reject minified/generated files using heuristics
    if is_probably_minified(filepath, content):
        return 0, None
