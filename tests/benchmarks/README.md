# SwampCastle Scale Benchmark Suite

106 tests that benchmark swampcastle at scale to validate real-world performance limits.

## Why

SwampCastle has strong academic scores (96.6% R@5 on LongMemEval) but no empirical data on how it behaves at scale. Key unknowns:
...
- **`generate_project_tree()`** — writes real files + `swampcastle.yaml` for `mine()` to ingest
