# SwampCastle Hooks — Auto-Save for Terminal AI Tools

These hook scripts make SwampCastle save automatically. No manual "save" commands needed.
...
- **`STATE_DIR`** — Where hook state is stored (defaults to `~/.swampcastle/hook_state/`)
- **`MEMPAL_DIR`** — Optional. Set to a conversations directory to auto-run `swampcastle mine <dir>` on each save trigger. Leave blank (default) to let the AI handle saving via the block reason message.
...
```bash
swampcastle mine <dir>               # Mine all files in a directory
swampcastle mine <dir> --mode convos # Mine conversation transcripts only
```
