...
def test_swampcastleignore_still_applies_when_gitignore_disabled(tmp_path):
    project = tmp_path
    (project / ".swampcastleignore").write_text("ignoreme.txt\n")
    (project / "ignoreme.txt").write_text("secret")
...
def test_project_swampcastleignore_negation_overrides_gitignore(tmp_path):
    project = tmp_path
