...
| `STATE_DIR` | `~/.swampcastle/hook_state/` | Where hook state and logs are stored |
| `MEMPAL_DIR` | (empty) | Set to a directory path to auto-run `swampcastle mine <dir>` on each save trigger |
...
Set `MEMPAL_DIR` to have the hooks automatically run `swampcastle mine` on a directory during each save trigger:
