#!/usr/bin/env python3
"""
onboarding.py — SwampCastle first-run setup.

Asks the user:
  1. How they're using SwampCastle (work / personal / combo)
...
Seeds the entity_registry with confirmed data so SwampCastle knows your world
from minute one — before a single session is indexed.

Usage:
    python3 -m swampcastle.onboarding
    or: swampcastle init
"""
...
def _ask_mode() -> str:
    _header("Welcome to SwampCastle")
    print("""
  SwampCastle is a personal memory system. To work well, it needs to know
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help SwampCastle distinguish project
...
def _generate_aaak_bootstrap(
    ...
    """
    castle_dir = Path(config_dir) if config_dir else Path.home() / ".swampcastle"
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by swampcastle init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
...
def run_onboarding(
  ...
  SwampCastle will check the context before treating them as person names.
...
""")
    ...
    print("\n  AAAK entity registry: ~/.swampcastle/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.swampcastle/critical_facts.md")
